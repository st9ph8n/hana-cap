...
/**
 * define view ProductViewSub as select from Products as prod {
 * productId as !\[Product_Id], ( select from
 * PurchaseOrder.Items as a { sum( grossAmount ) as SUM } where
 * a.product.productId = prod.productId\
 * ) as PO_SUM };
 *
 * define view ProductView as select from Products mixin {
 * PO_ORDERS : Association\[ \* ] to PurchaseOrder.ItemView on
 * PO_ORDERS.ProductId = $projection.!\[Product_Id]; } into {
 * productId as!\[Product_Id], name, desc, category
 * as!\[Product_Category], currency.code as!\[Product_Currency],
 * price as!\[Product_Price], typeCode as!\[Product_TypeCode],
 * weightMeasure as!\[Product_WeightMeasure], weightUnit
 * as!\[Product_WeightUnit], partner.ID as!\[Supplier_Id],
 ...
 * partner.address.country as!\[Supplier_Country], PO_ORDERS };
 */
